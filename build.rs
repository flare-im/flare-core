...
fn main() -> Result<()> {
    ...
    // 编译proto文件（自动生成protobuf序列化支持）
        if let Err(e) = config.compile_protos(&["frame.proto", "commands.proto"], &["proto/"]) {
            if files_exist {
                ...
                println!("cargo:warning=protoc compilation failed: {}, using pre-generated files", e);
                println!("cargo:warning=If you modify proto files, ensure protoc is installed: brew install protobuf");
            } else {
                // 如果文件不存在且编译失败，返回错误
    ...
    if std::path::Path::new(flare_core_path).exists() {
        ...
        let fixed_content = content
            .replace("super::super::commands::Command", "super::commands::Command")
