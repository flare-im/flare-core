...
impl ServerEventHandler for ChatRoomHandler {
    ...
    async fn on_connect(&self, connection_id: &str) -> Result<()> {
        ...
        // 初始化用户名（使用默认名称）
        {
            ...
            usernames
                .entry(connection_id.to_string())
                .or_insert_with(|| format!("用户_{}", &connection_id[..8.min(connection_id.len())]));
        }
...
async fn main() -> Result<()> {
    ...
    // ============================================================
    // 创建一个共享的 ConnectionManager（可选，用于多实例共享状态）
    // ============================================================
    ...
    let mut server = ObserverServerBuilder::new("0.0.0.0:8080", handler.clone())
        ...
        // 协商配置：设置服务端默认格式和压缩方式
        .with_default_format(SerializationFormat::Json) // 默认 JSON，可选 Protobuf
        .with_default_compression(flare_core::common::compression::CompressionAlgorithm::None) // 默认不压缩
        // 其他配置
...
- **`on_connect`**：连接建立完成时调用
  - 参数：`connection_id: &str` - 连接ID

- **`on_disconnect`**：连接断开时调用
  - 参数：`connection_id: &str` - 连接ID，`reason: Option<&str>` - 断开原因

### 2. ServerHandle 使用

通过 `ServerHandle` 可以进行消息发送和连接管理：

```rust
// 获取 ServerHandle
let server_handle: Arc<dyn ServerHandle> = Arc::new(DefaultServerHandle::new(manager_trait));
...
async fn broadcast_message(&self, frame: &Frame) {
    ...
    if let Some(ref handle) = *handle_guard {
        ...
        // 或广播给除指定连接外的所有连接
        handle.broadcast_except(frame, exclude_conn_id).await?;
        // 或发送给指定连接
        handle.send(frame, target_conn_id).await?;
    }
}
```
...
可以设置服务端默认的序列化格式和压缩方式：

```rust
.with_default_format(SerializationFormat::Json) // 或 SerializationFormat::Protobuf
.with_default_compression(CompressionAlgorithm::None) // 或 CompressionAlgorithm::Gzip
```
...
```bash
# 使用默认日志级别（info）
cargo run --example observer_server

# 使用 debug 级别查看详细信息
RUST_LOG=debug cargo run --example observer_server
```
