...
async fn main() -> Result<()> {
    ...
    info!("📱 设备信息：");
    info!("   平台: {:?} ({})", device_info.platform, device_info.platform.as_str());
    ...
    // ============================================================
    // 日志中间件
    let logging_middleware = Arc::new(LoggingMiddleware::new("ClientLogging")
        .with_level(LogLevel::Debug)) as ArcMessageMiddleware;
    
    // 性能监控中间件
    let metrics_middleware = Arc::new(MetricsMiddleware::new("ClientMetrics")) as ArcMessageMiddleware;
