...
/// 自定义客户端事件处理器
struct ClientAuthEventHandler;

#[async_trait::async_trait]
impl ConnectionEvent for ClientAuthEventHandler {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = ConnectionConfig::client(
        ...
        "ws://127.0.0.1:8080/ws".to_string(), // 示例地址
    )
    ...
    // 设置事件处理器
    let event_handler = Arc::new(ClientAuthEventHandler);
    ...
    // 发送认证请求
    let auth_request = Frame::auth_request("user123", "web", "token123");
    if let Err(e) = connection.send_message(auth_request).await {
        println!("发送认证请求失败: {}", e);
    }
