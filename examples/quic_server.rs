...
impl ChatRoomHandler {
    ...
    async fn get_stats(&self) -> (u64, u64, usize, usize) {
        let msg_count = self.message_count.load(std::sync::atomic::Ordering::Relaxed);
        let conn_count = self.connection_count.load(std::sync::atomic::Ordering::Relaxed);
...
impl ConnectionHandler for ChatRoomHandler {
    ...
    async fn on_connect(&self, connection_id: &str) -> Result<()> {
        ...
        // 发送欢迎消息
        let welcome_msg = format!("欢迎加入聊天室！您的连接 ID: {}", &connection_id[..8.min(connection_id.len())]);
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let mut quic_server = HybridServer::new(quic_config, handler.clone() as Arc<dyn ConnectionHandler>)?;
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            let (msg_count, conn_count, online_conn, online_users) = handler_for_periodic_stats.get_stats().await;
            ...
            if let Some(ref handle) = *handle_guard {
                ...
                if current_conn > 0 {
                    info!("📊 统计信息:");
                    info!("   - 当前在线连接: {} (用户: {})", current_conn, current_users);
    ...
    // 显示最终统计
    let (final_msg_count, final_conn_count, final_online_conn, final_online_users) = handler_for_final_stats.get_stats().await;
    info!("");
    info!("📊 最终统计:");
    info!("   - 当前在线连接: {} (用户: {})", final_online_conn, final_online_users);
