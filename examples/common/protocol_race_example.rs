...
/// 协议竞速事件处理器
#[derive(Debug)]
pub struct ProtocolRaceEventHandler {
    pub name: String,
}

#[async_trait::async_trait]
impl ConnectionEvent for ProtocolRaceEventHandler {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let websocket_config = ConnectionConfig::client(
        "protocol_race_websocket".to_string(),
    ...
    let quic_event_handler = Arc::new(ProtocolRaceEventHandler::new("QUIC客户端".to_string()));
    ...
    let websocket_event_handler = Arc::new(ProtocolRaceEventHandler::new("WebSocket客户端".to_string()));
    ...
    // 选择连接更快的协议
    let (mut client_connection, protocol_name, connect_time) = match (quic_result, websocket_result) {
    ...
    let test_message = Frame::new(
        ...
        format!("Hello from {} client with protocol race and Protobuf!", protocol_name).as_bytes().to_vec(),
    );
