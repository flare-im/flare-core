...
use flare_core::{
    common::{
        connections::{
            ...
            websocket::WebSocketClientConn,  // 使用WebSocket客户端连接
        },
        protocol::{
            frame::Frame,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("用法: cargo run --example simple_chat_client <用户名>");
        eprintln!("示例: cargo run --example simple_chat_client user1");
        std::process::exit(1);
    }
    ...
    println!("🚀 启动WebSocket聊天室客户端，用户名: {}", username);
    
    // 创建客户端配置
    let mut config = ConnectionConfig::default();
    config.transport = Transport::WebSocket;
    config.remote_addr = Some("127.0.0.1:9005".to_string());
    config.enable_tls = false; // 禁用TLS
    ...
    // 创建WebSocket客户端连接
    let client = WebSocketClientConn::from_config(config);
...
/// 发送聊天消息的辅助函数
fn send_chat_message(client: &WebSocketClientConn, sender: String, content: String) -> Result<(), Box<dyn std::error::Error>> {
