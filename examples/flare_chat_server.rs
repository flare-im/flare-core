...
use flare_core::common::message::{
    ...
    FunctionProcessor, MessageContext,
};
use flare_core::common::protocol::{Frame, MessageCommand, frame_with_message_command, generate_message_id, Reliability, SerializationFormat};
...
async fn main() -> Result<()> {
    ...
    let validation_middleware = Arc::new(ValidationMiddleware::new(
        ...
        |frame: &Frame| -> Result<()> {
            ...
            if frame.message_id.is_empty() {
                return Err(FlareError::message_format_error("Message ID is empty".to_string()));
            }
            Ok(())
        },
    ...
    // 日志中间件（高优先级，最先执行）
    let logging_middleware = Arc::new(LoggingMiddleware::new("ChatLogging")
        .with_level(LogLevel::Info)) as ArcMessageMiddleware;
    
    // 性能监控中间件
    let metrics_middleware = Arc::new(MetricsMiddleware::new("ChatMetrics")) as ArcMessageMiddleware;
    ...
    let echo_processor = Arc::new(FunctionProcessor::new("EchoProcessor", |ctx: &MessageContext| {
            ...
            if let Some(cmd) = &frame.command {
                if let Some(CommandType::Message(msg_cmd)) = &cmd.r#type {
                    ...
                    if message_text.starts_with("echo: ") {
                        ...
                        let echo_frame = frame_with_message_command(
                            echo_cmd,
