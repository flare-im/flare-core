...
impl MessageHandler for DefaultMessageHandler {
    fn handle_message(&self, _user_id: String, _frame: Frame) -> Result<(), FlareError> {
        // 默认实现：简单记录日志
        tracing::debug!("处理消息");
        Ok(())
    }
...
impl MessageHandlerWrapper {
    ...
    pub fn handle_message(&self, user_id: String, frame: Frame) -> Result<(), FlareError> {
        if let Some(handler) = &self.sync_handler {
            handler.handle_message(user_id, frame)
            ...
            // 默认处理
            DefaultMessageHandler::default().handle_message(user_id, frame)
        }
    }
}