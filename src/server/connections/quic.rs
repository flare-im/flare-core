...
use crate::common::connections::quic::QuicServerConn;
...
impl QuicServerConnection {
    ...
    pub fn from_config(config: ConnectionConfig) -> Self {
        ...
        let connection = Arc::new(QuicServerConn::from_config(quic_config));
        Self { connection }
    }
    ...
    /// # 返回值
    /// QUIC服务端连接实例
    pub fn from_quic_connection(conn: quinn::Connection, config: ConnectionConfig) -> Self {
        // 确保配置指定了QUIC传输协议
        let mut quic_config = config.clone();
        quic_config.transport = Transport::Quic;
        
        let connection = Arc::new(QuicServerConn::from_quinn_connection(conn, quic_config));
        Self { connection }
    }
...
impl BaseConnection for QuicServerConnection {
...
impl ServerConnection for QuicServerConnection {
