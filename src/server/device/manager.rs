...
use crate::common::device::{DeviceInfo, DevicePlatform, DeviceConflictStrategy};
...
impl DeviceManager {
    ...
    pub async fn add_device(
        ...
        let conflicts = match self.conflict_strategy.check_conflict(
            platform.clone(),
            &existing_platforms,
        ) {
            Ok(_) => {
    ...
    pub async fn get_user_devices(&self, user_id: &str) -> Vec<DeviceInfo> {
        ...
        user_devices
            .get(user_id)
            .map(|devices| {
                devices.values().map(|d| d.device_info.clone()).collect()
            })
            .unwrap_or_default()
    }
    ...
    pub async fn get_device_connection(&self, user_id: &str, device_id: &str) -> Option<String> {
        ...
        user_devices
            .get(user_id)
            .and_then(|devices| {
                devices.values().find(|d| d.device_info.device_id == device_id)
