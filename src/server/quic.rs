//! QUIC 服务端实现
//!
//! 提供基于 QUIC 协议的服务端功能实现
//!
//! # 特性
//!
//! - 基于 [quinn](https://crates.io/crates/quinn) 库实现
//! - 支持 TLS 加密（使用自签名证书）
//! - 低延迟、高可靠性
//! - 与连接管理器无缝集成
...
//! # 核心组件
//!
//! - [QuicServer](struct.QuicServer.html): QUIC 服务端主类
...
//! 
//! // 创建配置
//! let config = ConnectionConfig::server("quic_server".to_string(), "127.0.0.1:8081".to_string());
//! 
//! // 创建 QUIC 服务端
//! let server = QuicServer::new(config, connection_manager);
//! ```
...
pub struct QuicServer<T: ConnectionManager> {
    /// 配置
...
impl<T: ConnectionManager + 'static> QuicServer<T> {
    /// 创建新的 QUIC 服务端
    ///
    /// # 参数
    ///
...
impl<T: ConnectionManager + 'static> ServerService for QuicServer<T> {
    /// 启动 QUIC 服务
    ///
