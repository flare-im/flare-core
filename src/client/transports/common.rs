...
impl ClientMessageObserver {
    ...
    /// 通知所有观察者（内部辅助函数）
    fn notify_observers(observers: &Arc<std::sync::Mutex<Vec<ArcObserver>>>, event: &ConnectionEvent) {
...
impl ClientConnectionHelper {
    ...
    pub async fn send_frame_internal(
        ...
        let conn = connection.ok_or_else(|| {
            FlareError::connection_failed("Not connected".to_string())
        })?;
    ...
    pub async fn disconnect_internal(
        ...
        core.state_manager.set_state(crate::client::connection::ConnectionState::Disconnecting);
