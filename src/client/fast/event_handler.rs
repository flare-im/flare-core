...
/// FastClient事件处理器trait
pub trait FastClientEventHandlerTrait: Send + Sync {
    /// 连接建立时触发
...
/// FastClient事件处理器
pub struct FastClientEventHandler {
    handler: Option<Arc<dyn FastClientEventHandlerTrait>>,
}

impl FastClientEventHandler {
    pub fn new() -> Self {
        Self { handler: None }
    }
    
    pub fn with_handler(handler: Arc<dyn FastClientEventHandlerTrait>) -> Self {
        Self { handler: Some(handler) }
    }
    
    /// 设置事件处理器
    pub fn set_handler(&mut self, handler: Arc<dyn FastClientEventHandlerTrait>) {
        self.handler = Some(handler);
    }
...
impl ConnectionEvent for FastClientEventHandler {
    fn on_connected(&self) {
    ...
    fn on_reconnect_failed(&self, err: FlareError) {
        if let Some(handler) = &self.handler {
            handler.on_reconnect_failed(err);
        } else {
            tracing::error!("重连失败: {:?}", err);
        }
    }
}

impl Default for FastClientEventHandler {
    fn default() -> Self {
        Self::new()
    }
}