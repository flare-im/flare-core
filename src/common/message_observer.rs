...
pub trait MessageObserver: Send + Sync {
    ...
    async fn handle_frame(&self, frame: &Frame, connection_id: Option<&str>) -> Result<Option<Frame>> {
        if let Some(cmd) = &frame.command {
            match &cmd.r#type {
                Some(crate::common::protocol::flare::core::commands::command::Type::System(sys_cmd)) => {
                    self.handle_system_command(&sys_cmd, frame, connection_id).await
                }
                Some(crate::common::protocol::flare::core::commands::command::Type::Message(msg_cmd)) => {
                    self.handle_message_command(&msg_cmd, frame, connection_id).await
                }
                Some(crate::common::protocol::flare::core::commands::command::Type::Notification(notif_cmd)) => {
                    self.handle_notification_command(&notif_cmd, frame, connection_id).await
                }
                Some(crate::common::protocol::flare::core::commands::command::Type::Custom(custom_cmd)) => {
                    self.handle_custom_command(&custom_cmd, frame, connection_id).await
                }
                None => Ok(None),
            }
        } else {
    ...
    /// 
    /// 当连接状态发生变化时调用
    async fn handle_connection_event(&self, _event: &ConnectionEvent, _connection_id: Option<&str>) -> Result<()> {
        Ok(())
    }
}
